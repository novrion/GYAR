...
void GenerateMoves(const U64 kBB[13], U64 moves[100], const bool kSide) {
	...
	if (kSide) {
		...
		while (piece_bitboard) WPawnMoves(PopLsb(piece_bitboard), kWBlock, kBBlock, moves);
		...
		while (piece_bitboard) WKingMoves(PopLsb(piece_bitboard), kWBlock, kBBlock, moves);
	}

	else {

		U64 piece_bitboard = kBB[7];
		while (piece_bitboard) BPawnMoves(PopLsb(piece_bitboard), kWBlock, kBBlock, moves);
		...
		while (piece_bitboard) BKingMoves(PopLsb(piece_bitboard), kWBlock, kBBlock, moves);
	}
}
