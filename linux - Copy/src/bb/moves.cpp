...
inline void WKingMoves(const int kSquare, const U64 kWBlock, const U64 kBBlock, U64 moves[100], const U64 kBB[13]) {
	...
	// if (w_long_castle && !(kWCastleLongSpaceMask & (kWBlock | kBBlock)) && kBB[4] & (1ULL << 56))// && !(kBB[7] & kWCastleLongPawnMask) && !(kBB[8] & kWCastleLongKnightMask) && !(kBB[12] & kWCastleLongKingMask) && kBB[4] & (1ULL << 56)
	// {
	...
	// if (w_short_castle && !(kWCastleShortSpaceMask & (kWBlock | kBBlock)) && kBB[4] & (1ULL << 63))// && !(kBB[7] & kWCastleShortPawnMask) && !(kBB[8] & kWCastleShortKnightMask) && !(kBB[12] & kWCastleShortKingMask) && kBB[4] & (1ULL << 63)
	// {
	// AddMove(moves, ENCODE_MOVE(60, 62, 6, 0, 0, 0, 0, 2));
...
inline void BKingMoves(const int kSquare, const U64 kWBlock, const U64 kBBlock, U64 moves[100], const U64 kBB[13]) {
	...
	// Castle V2
	if (GET_UTILITY_B_LONG_CASTLE(kBB[0]) && kBB[10] & 1ULL && !(kBCastleLongSpaceMask & (kWBlock | kBBlock))) {
		AddMove(moves, ENCODE_MOVE(4, 2, 12, 0, 0, 0, 0, 3));
	}
	...
	// if (b_long_castle && !(kBCastleLongSpaceMask & (kWBlock | kBBlock)) && kBB[10] & 1ULL //&& !(kBB[1] & kBCastleLongPawnMask) && !(kBB[2] & kBCastleLongKnightMask) && !(kBB[6] & kWCastleLongKingMask) && kBB[10] & 1ULL
	// ) {
	// 		AddMove(moves, ENCODE_MOVE(4, 2, 12, 0, 0, 0, 0, 3));
	...
	// if (b_short_castle && !(kBCastleShortSpaceMask & (kWBlock | kBBlock)) && kBB[10] & (1ULL << 7) //&& !(kBB[1] & kBCastleShortPawnMask) && !(kBB[2] & kBCastleShortKnightMask) && !(kBB[6] & kWCastleShortKingMask) && kBB[10] & (1ULL << 7)
	// ) {
	// 	AddMove(moves, ENCODE_MOVE(4, 6, 12, 0, 0, 0, 0, 4));
