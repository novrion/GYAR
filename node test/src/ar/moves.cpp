  ...
  if (kSide) {
    for (int x = 0; x < 8; x++) {
      for (int y = 0; y < 8; y++) {
        ...
        else if (b.board[x][y] == 10) WPawnMoves(b, moves, x, y);
        else if (b.board[x][y] == 30) WKnightMoves(b, moves, x, y);
        else if (b.board[x][y] == 31) WBishopMoves(b, moves, x, y);
        else if (b.board[x][y] == 50) WRookMoves(b, moves, x, y);
        else if (b.board[x][y] == 90) WQueenMoves(b, moves, x, y);
        else if (b.board[x][y] == 1000) WKingMoves(b, moves, x, y);
      }
    }
  }
    ...
    for (int x = 0; x < 8; x++) {
      for (int y = 0; y < 8; y++) {
        if (b.board[x][y] == 0) continue;
        else if (b.board[x][y] == -10) BPawnMoves(b, moves, x, y);
        else if (b.board[x][y] == -30) BKnightMoves(b, moves, x, y);
        else if (b.board[x][y] == -31) BBishopMoves(b, moves, x, y);
        else if (b.board[x][y] == -50) BRookMoves(b, moves, x, y);
        else if (b.board[x][y] == -90) BQueenMoves(b, moves, x, y);
        else if (b.board[x][y] == -1000) BKingMoves(b, moves, x, y);
      }
    }
  }
}
...
inline bool CastleDiagonalInCheck(Board& b, const int kPosX, const bool kSide) {
  ...
  if (kSide) {
    ...
    for (int x = kPosX - 1, y = 1; x > -1 && y < 8; --x, ++y) {
      ...
      else if (b.board[x][y] == -31 || b.board[x][y] == -90) return false;
      else break;
    }

    for (int y = 1; y < 8; ++y) {
      if (!b.board[kPosX][y]) continue;
      else if (b.board[kPosX][y] == -50 || b.board[kPosX][y] == -90) return false;
      else break;
    }
...
inline void WPawnMoves(Board& b, Move moves[100], const int kPosX, const int kPosY) {
  ...
  if (kPosY == 6) {
    ...
    if (kPosX + 1 < 8 && b.board[kPosX + 1][kPosY + 1] < 0) {
      AddMove(moves, Move(kPosX, kPosY, kPosX + 1, kPosY + 1, b.board[kPosX + 1][kPosY + 1], false, false, 30, 0));
      AddMove(moves, Move(kPosX, kPosY, kPosX + 1, kPosY + 1, b.board[kPosX + 1][kPosY + 1], false, false, 31, 0));
      AddMove(moves, Move(kPosX, kPosY, kPosX + 1, kPosY + 1, b.board[kPosX + 1][kPosY + 1], false, false, 50, 0));
      AddMove(moves, Move(kPosX, kPosY, kPosX + 1, kPosY + 1, b.board[kPosX + 1][kPosY + 1], false, false, 90, 0));
    }
    ...
    if (!b.board[kPosX][kPosY + 1]) {
      AddMove(moves, Move(kPosX, kPosY, kPosX, kPosY + 1, 0, false, false, 30, 0));
      AddMove(moves, Move(kPosX, kPosY, kPosX, kPosY + 1, 0, false, false, 31, 0));
      AddMove(moves, Move(kPosX, kPosY, kPosX, kPosY + 1, 0, false, false, 50, 0));
      AddMove(moves, Move(kPosX, kPosY, kPosX, kPosY + 1, 0, false, false, 90, 0));
    }
  }
    ...
    if (b.en_passant_x != -1 && b.en_passant_y == kPosY) {
      ...
      if (kPosX == b.en_passant_x + 1) {
        AddMove(moves, Move(kPosX, kPosY, kPosX - 1, kPosY + 1, 10, true, false, 0, 0));
...
inline void WKingMoves(Board& b, Move moves[100], const int kPosX, const int kPosY) {
  ...
  // Castle
  if (b.w_l_castle && b.board[0][0] == 50 && !b.board[1][0] && !b.board[2][0] && !b.board[3][0]) {
    if (CastleDiagonalInCheck(b, 1, true) && CastleDiagonalInCheck(b, 2, true) && CastleDiagonalInCheck(b, 3, true) && CastleDiagonalInCheck(b, 4, true) &&
      !(b.board[0][1] == -10 || b.board[1][1] == -10 || b.board[2][1] == -10 || b.board[3][1] == -10 || b.board[4][1] == -10 || b.board[5][1] == -10) &&
      !(b.board[0][2] == -30 || b.board[1][2] == -30 || b.board[2][2] == -30 || b.board[3][2] == -30 || b.board[4][2] == -30 || b.board[5][2] == -30) &&
      !(b.board[0][1] == -30 || b.board[1][1] == -30 || b.board[2][1] == -30 || b.board[3][1] == -30 || b.board[4][1] == -30 || b.board[5][1] == -30 || b.board[6][1] == -30) &&
      !(b.board[1][1] == -1000 || b.board[2][1] == -1000)) {

      AddMove(moves, Move(kPosX, kPosY, kPosX - 2, kPosY, 0, false, false, 0, 1));
    }
  }

  if (b.w_r_castle && b.board[7][0] == 50 && !b.board[5][0] && !b.board[6][0]) {
    if (CastleDiagonalInCheck(b, 4, true) && CastleDiagonalInCheck(b, 5, true) && CastleDiagonalInCheck(b, 6, true) &&
      !(b.board[3][1] == -10 || b.board[4][1] == -10 || b.board[5][1] == -10 || b.board[6][1] == -10 || b.board[7][1] == -10) &&
      !(b.board[3][2] == -30 || b.board[4][2] == -30 || b.board[5][2] == -30 || b.board[6][2] == -30 || b.board[7][2] == -30) &&
      !(b.board[2][1] == -30 || b.board[3][1] == -30 || b.board[4][1] == -30 || b.board[6][1] == -30 || b.board[7][1] == -30) &&
      !(b.board[6][1] == -1000)) {
...
inline void BKingMoves(Board& b, Move moves[100], const int kPosX, const int kPosY) {
  ...
  // Castle
  if (b.b_l_castle && b.board[0][7] == -50 && !b.board[1][7] && !b.board[2][7] && !b.board[3][7]) {
    if (CastleDiagonalInCheck(b, 1, false) && CastleDiagonalInCheck(b, 2, false) && CastleDiagonalInCheck(b, 3, false) && CastleDiagonalInCheck(b, 4, false) &&
      !(b.board[0][6] == 10 || b.board[1][6] == 10 || b.board[2][6] == 10 || b.board[3][6] == 10 || b.board[4][6] == 10 || b.board[5][6] == 10) &&
      !(b.board[0][5] == 30 || b.board[1][5] == 30 || b.board[2][5] == 30 || b.board[3][5] == 30 || b.board[4][5] == 30 || b.board[5][5] == 30) &&
      !(b.board[0][6] == 30 || b.board[1][6] == 30 || b.board[2][6] == 30 || b.board[3][6] == 30 || b.board[4][6] == 30 || b.board[5][6] == 30 || b.board[6][6] == 30) &&
      !(b.board[1][6] == 1000 || b.board[2][6] == 1000)) {

      AddMove(moves, Move(kPosX, kPosY, kPosX - 2, kPosY, 0, false, false, 0, 3));
    }
  }

  if (b.b_r_castle && b.board[7][7] == -50 && !b.board[5][7] && !b.board[6][7]) {
    if (CastleDiagonalInCheck(b, 4, false) && CastleDiagonalInCheck(b, 5, false) && CastleDiagonalInCheck(b, 6, false) &&
      !(b.board[3][6] == 10 || b.board[4][6] == 10 || b.board[5][6] == 10 || b.board[6][6] == 10 || b.board[7][6] == 10) &&
      !(b.board[3][5] == 30 || b.board[4][5] == 30 || b.board[5][5] == 30 || b.board[6][5] == 30 || b.board[7][5] == 30) &&
      !(b.board[2][6] == 30 || b.board[3][6] == 30 || b.board[4][6] == 30 || b.board[6][6] == 30 || b.board[7][6] == 30) &&
      !(b.board[6][6] == 1000)) {
